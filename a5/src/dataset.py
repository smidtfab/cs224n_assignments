...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncated_len = random.randint(4, int(self.block_size * 7 / 8 + 1))
        truncated_doc = doc[:truncated_len]
        # 2. Break the (truncated) document into three substrings: [prefix] [masked_content] [suffix]
        masked_content_length = round(random.gauss(truncated_len / 4, 2))
        masked_content_idx = random.randint(1, truncated_len - masked_content_length)
        prefix = truncated_doc[:masked_content_idx]
        masked_content = truncated_doc[masked_content_idx:masked_content_idx + masked_content_length]
        suffix = truncated_doc[masked_content_idx + masked_content_length:]
        ...
        x = masked_string[:-1]
        y = masked_string[1:]
        # 5. encode the resulting input and output strings as Long tensors and return the resulting data point
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in y], dtype=torch.long)
        """
        doc = self.data[idx]
